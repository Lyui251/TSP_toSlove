...
def distance_calculation(i_index, j_index):
    ...
    outcom = round(
        m.cos(x1_radians - x2_radians) * m.cos(y1_radians) * m.cos(y2_radians) + m.sin(y1_radians) * m.sin(y2_radians),
        4)
...
def calculate_probability(tau_m, eta_m, taboo_l, current_index):
    ...
    for i in range(0, len(taboo_l)):
        if taboo_l[i] == 1:  # 根据禁忌表中，可以访问的城市，访问过的城市为0，没有的为1，1表示可以访问
            numerator_lst[i] = (tau_m[current_index, i] ** alpha) * (
                    eta_m[current_index, i] ** beta) + 0.1 ** 50  # 分子的计算
...
if __name__ == "__main__":
    # 导入数据
    data_df = pd.read_csv(r"E:\TSP_toSlove\34城市坐标.txt")
            ...
            for k in range(0, K):  # 2-邻边算法循环，假设进行K轮改进
                ...
                for i in range(1, len(path) - 4):  # len(path)=102 ; city_number = 101; len(dis_mtx)=101
                    for j in range(i + 2, len(path) - 2):
                        if (dis_mtx[path[i], path[j]] + dis_mtx[path[i + 1], path[j + 1]]) < (
                                dis_mtx[path[i], path[i + 1]] + dis_mtx[path[j], path[j + 1]]):
                            path[i + 1:j + 1] = path[
                                                j:i:-1]  # [i+1:j+1]包括了[i+1,i+2,...,j];[j:i:-1]包括了[j,j-1,...,i+1],切片的左闭右开特性
                            flag = 1
                if flag == 0:
                    break
    ...
    print()
    print(f"最短距离为{min(shortest_dis_lst):0.4f}千米")
    ...
    length_result.to_excel(r'E:\TSP_toSlove\result\Length_Result.xlsx', 'sheet1')
    path_result.to_excel(r'E:\TSP_toSlove\result\Path_Result.xlsx', 'sheet1')
    file_save = open(r'E:\TSP_toSlove\result\optimal_path.txt', 'w').close()
    file_save_handle = open(r'E:\TSP_toSlove\result\optimal_path.txt', mode='a')
    ...
    plt.xlim(85, 130, 1)  # x轴的刻度范围
    plt.ylim(15, 50, 1)  # y轴的刻度范围
    plt.xlabel('经度', fontproperties="simhei")  # x轴的标题
    plt.ylabel('纬度', fontproperties="simhei")  # y轴的标题
    # 绘制各个点及点所代表地点名称
    for i_index in optimal_policy:
        i_index -= 1
        x1 = data_df.loc[i_index, 'longitude']
        y1 = data_df.loc[i_index, 'latitude']
        plt.text(x1, y1, '城市' + str(i_index), color='#0085c3', fontproperties="simhei")
        plt.plot(x1, y1, 'o', color='#0085c3')
    # 连接各个点
    for i, i_index in enumerate(optimal_policy):
        i_index -= 1
        if i == len(optimal_policy) - 1:    break
        x1 = data_df.loc[i_index, 'longitude']
        y1 = data_df.loc[i_index, 'latitude']

        x2 = data_df.loc[optimal_policy[i + 1] - 1, 'longitude']
        y2 = data_df.loc[optimal_policy[i + 1] - 1, 'latitude']
        start = (x1, x2)
        end = (y1, y2)
        plt.plot(start, end, color='#0085c3')
    plt.savefig(r"E:\TSP_toSlove\figure\路径.png", dpi=100)
    plt.show()
